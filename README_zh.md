...
| `seed` | INT | `0` | `0` 使用当前随机状态；正值可重复。 |
| `emotion` | COMBO | `none` | 可选的全局情感，前置到每个块。 |
| `style` | COMBO | `none` | 可选的全局风格。 |
...
下拉控制是可选的便捷方式。如果 `emotion`、`style`、`speed`、`pitch` 和 `expressiveness` 都设为 `none`，直接在 `text` 中输入的内联标签仍然生效。
...
长文本分块会保留标签并避免在 `<|...|>` 内部切割。当下拉控制设为 `none` 时，分块器会将最新的活跃标签状态传递到后续块中。
...
- 当前块编号和预览文本；
- 多说话人轮次编号和说话人 ID；
- 周期性音频 token 进度，如 `Higgs v3 audio tokens [##----------------------] 128/2048`。

ComfyUI UI 进度条也会在块/轮次级别更新。
